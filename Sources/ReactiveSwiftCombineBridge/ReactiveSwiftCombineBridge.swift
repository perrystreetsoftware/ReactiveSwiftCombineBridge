// Adapted from:
// https://github.com/ReactiveCocoa/ReactiveSwift/pull/776/files
...
final class ProducerSubscription<S: Subscriber>: Combine.Subscription {
    ...
    let subscriber: S
    let base: SignalProducer<Input, Failure>
    let state: Atomic<State>

    init(subscriber: S, base: SignalProducer<Input, Failure>) {
